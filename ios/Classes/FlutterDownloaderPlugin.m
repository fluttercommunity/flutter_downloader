...
- (instancetype)init:(NSObject <FlutterPluginRegistrar> *)registrar; {
    if (self = [super init]) {
        ...
        if(_isolate) {
            ...
            // configure a session that only allows downloads via wifi
            NSURLSessionConfiguration *wifiSessionConfiguration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:[NSString stringWithFormat:@"%@.wifi.download.background.%f", NSBundle.mainBundle.bundleIdentifier, [[NSDate date] timeIntervalSince1970]]];
            ...
            _wifiSession = [NSURLSession sessionWithConfiguration:wifiSessionConfiguration delegate:self delegateQueue:nil];
            ...
            if (debug) {
                ...
                NSLog(@"init wifi only NSURLSession with id: %@", [[_wifiSession configuration] identifier]);
            }
        }
...
- (void)pauseTaskWithId:(NSString *)taskId {
    ...
    for (NSURLSessionDownloadTask *download in [self getTasksForAllSessions]) {
        NSURLSessionTaskState state = download.state;
        NSString *taskIdValue = [weakSelf identifierForTask:download ofSession: self.currentSession];
        NSString *taskIdWifi = [weakSelf identifierForTask:download ofSession: self.wifiSession];
        if (([taskId isEqualToString:taskIdValue] || [taskId isEqualToString:taskIdWifi]) && (state == NSURLSessionTaskStateRunning)) {
...
- (void)cancelTaskWithId:(NSString *)taskId {
    ...
    for (NSURLSessionDownloadTask *download in [self getTasksForAllSessions]) {
        NSURLSessionTaskState state = download.state;
        NSString *taskIdValue = [self identifierForTask:download ofSession: self.currentSession];
        NSString *taskIdWifi = [self identifierForTask:download ofSession: self.wifiSession];
        if (([taskId isEqualToString:taskIdValue] || [taskId isEqualToString:taskIdWifi]) && (state == NSURLSessionTaskStateRunning)) {
            [download cancel];
...
- (NSArray<NSURLSessionDownloadTask *> *)getTasksForAllSessions {
    NSArray<NSURLSessionDownloadTask *> *allDownloads;
    ...
    [[self currentSession] getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> *data, NSArray<NSURLSessionUploadTask *> *uploads, NSArray<NSURLSessionDownloadTask *> *downloads) {
        [allDownloads arrayByAddingObjectsFromArray:downloads];
    }];

    [[self wifiSession] getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> *data, NSArray<NSURLSessionUploadTask *> *uploads, NSArray<NSURLSessionDownloadTask *> *downloads) {
        [allDownloads arrayByAddingObjectsFromArray:downloads];
    }];
...
- (void)cancel:(NSURLSessionDownloadTask *)download ofSession:(NSURLSession *)session {
    NSURLSessionTaskState state = download.state;
    if (state == NSURLSessionTaskStateRunning) {
        [download cancel];
        NSString *taskId = [self identifierForTask:download ofSession:session];
...
- (void)enqueueMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    _runningTaskById[taskId] = [@{KEY_URL: urlString,
            KEY_FILE_NAME: fileName,
            KEY_SAVED_DIR: savedDir,
            KEY_HEADERS: headers,
            KEY_SHOW_NOTIFICATION: showNotification,
            KEY_OPEN_FILE_FROM_NOTIFICATION: openFileFromNotification,
            KEY_RESUMABLE: @(NO),
            KEY_STATUS: @(STATUS_ENQUEUED),
            KEY_PROGRESS: @(0),
            KEY_ALLOW_CELLULAR: @(allowCellular)} mutableCopy];
...
- (void)resumeMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (taskDict != nil) {
        ...
        if ([status intValue] == STATUS_PAUSED) {
            ...
            if (resumeData != nil) {
                bool wifiOnly = (bool)taskDict[KEY_ALLOW_CELLULAR];
                NSURLSessionDownloadTask *task = [(wifiOnly ? self.wifiSession : self.currentSession) downloadTaskWithResumeData:resumeData];
...
- (void)removeMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    __typeof__(self) __weak weakSelf = self;
    NSString *taskId = call.arguments[KEY_TASK_ID];
    Boolean shouldDeleteContent = (Boolean) [call.arguments[@"should_delete_content"] boolValue];
    NSDictionary *taskDict = [self loadTaskWithId:taskId];
    if (taskDict != nil) {
        NSNumber *status = taskDict[KEY_STATUS];
        if ([status intValue] == STATUS_ENQUEUED || [status intValue] == STATUS_RUNNING) {
            ...
            for (NSURLSessionDownloadTask *download in [self getTasksForAllSessions]) {
                ...
                NSString *taskIdValue = [weakSelf identifierForTask:download ofSession:self.currentSession];
                NSString *taskIdWifi = [weakSelf identifierForTask:download ofSession:self.wifiSession];
                if (([taskId isEqualToString:taskIdValue] || [taskIdWifi isEqualToString:taskIdValue]) && (state == NSURLSessionTaskStateRunning)) {
                    [download cancel];
                    [weakSelf sendUpdateProgressForTaskId:taskId inStatus:@(STATUS_CANCELED) andProgress:@(-1)];
                    [weakSelf executeInDatabaseQueueForTask:^{
...
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session {
    ...
    [[self wifiSession] getTasksWithCompletionHandler:^(NSArray *dataTasks, NSArray *uploadTasks, NSArray *downloadTasks) {
        if ([downloadTasks count] == 0) {
            if (debug) {
                NSLog(@"all download tasks have been finished");
            }
